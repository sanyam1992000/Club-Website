...
from ClubWebsite import settings
...
@login_required
def logout_user(request):	#added when updated to django==3
    logout(request)
...
def change_password(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Please correct the old password and new password')
    else:
